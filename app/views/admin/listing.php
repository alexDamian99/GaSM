    ...
    <main>
        <section class="container">
                        ...
                        <?php foreach($data['elements'] as $element) {?>
                            <tr>
                                ...
                                <?php if($data['type'] === "Campaigns") { ?>
                                    <td><a target="_blank" href="<?=getenv('path_to_public') . '/campaigns/id/' . $element['id']?>"><i class="fa fa-window-restore" aria-hidden="true"></i></a></td>
                                <?php } ?>
                                <td><button onclick="deleteElement('<?=$data['type']?>', <?=$element['id']?>)"><i class="fa fa-trash" aria-hidden="true"></i></button></td>
                            </tr>
                        <?php }?>
                    </tbody>
                </table>
            ...
            <div class="pagination">
                <a class="<?php if($page <= 1){ echo "disabled"; }?>" href="<?= getenv('path_to_public') . "/admin/" . strtolower($data['type']) . "/" . ($page - 1) ?>"><</a>
                <?php for($i = $start_page; $i <= $stop_page; $i++) {?>
                    <a class="<?=($page == $i) ? "page-active" : ''?>" href="<?= getenv('path_to_public') . "/admin/" . strtolower($data['type']) . "/" . $i ?>"> <?=$i?> </a>
                <?php } ?>
                <a class="<?php if($page >= $total_pages){ echo "disabled"; }?>" href="<?= getenv('path_to_public') . "/admin/" . strtolower($data['type']) . "/" . ($page + 1) ?>">></a>
            </div>
        </section>
    </main>
</body>
